...
import { emit, type SimStep } from '../steps'

export function engineerScript(ctx: MissionContext): SimStep[] {
  return ctx.attempt === 0 ? firstPass(ctx) : rework(ctx)
}

function firstPass(ctx: MissionContext): SimStep[] {
...
function rework(ctx: MissionContext): SimStep[] {
